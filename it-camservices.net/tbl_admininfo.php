...
class ctbl_admin {
	...
	function ctbl_admin() {
		...
		// admin_id
		$this->admin_id = new cField('tbl_admin', 'tbl_admin', 'x_admin_id', 'admin_id', '`admin_id`', 19, -1, FALSE, '`admin_id`', FALSE);
		$this->admin_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['admin_id'] =& $this->admin_id;

		// username
		$this->username = new cField('tbl_admin', 'tbl_admin', 'x_username', 'username', '`username`', 200, -1, FALSE, '`username`', FALSE);
		$this->fields['username'] =& $this->username;

		// password
		$this->password = new cField('tbl_admin', 'tbl_admin', 'x_password', 'password', '`password`', 200, -1, FALSE, '`password`', FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `tbl_admin` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= ew_QuotedName('admin_id') . '=' . ew_QuotedValue($rs['admin_id'], $this->admin_id->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@admin_id@", ew_AdjustSql($this->admin_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("tbl_adminview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "tbl_adminadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("tbl_adminedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("tbl_adminadd.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=tbl_admin" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
