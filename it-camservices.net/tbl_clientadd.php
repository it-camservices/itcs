...
// Create page object
$tbl_client_add = new ctbl_client_add();
$Page =& $tbl_client_add;

// Page init
$tbl_client_add->Page_Init();

// Page main
$tbl_client_add->Page_Main();
?>
...
// Create page object
var tbl_client_add = new ew_Page("tbl_client_add");

// page properties
tbl_client_add.PageID = "add"; // page ID
tbl_client_add.FormID = "ftbl_clientadd"; // form ID
var EW_PAGE_ID = tbl_client_add.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_client_add.ValidateForm = function(fobj) {
...
tbl_client_add.Form_CustomValidate =  
...
tbl_client_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_client_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_client->TableCaption() ?><br><br>
<a href="<?php echo $tbl_client->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$tbl_client_add->ShowMessage();
?>
<form name="ftbl_clientadd" id="ftbl_clientadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return tbl_client_add.ValidateForm(this);">
<p>
...
class ctbl_client_add {
	...
	// Page object name
	var $PageObjName = 'tbl_client_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $tbl_client->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$tbl_client->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $tbl_client->CurrentAction = "C"; // Copy record
		  } else {
		    $tbl_client->CurrentAction = "I"; // Display blank record
		...
		switch ($tbl_client->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$tbl_client->SendEmail = TRUE; // Send email on add success
		...
		$tbl_client->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $tbl_client;
		$tbl_client->photo->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		} elseif ($tbl_client->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $tbl_client->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($tbl_client->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$tbl_client->client_id->setDbValue($conn->Insert_ID());
			$rsnew['client_id'] = $tbl_client->client_id->DbValue;

			// Call Row Inserted event
			$tbl_client->Row_Inserted($rsnew);
		}
