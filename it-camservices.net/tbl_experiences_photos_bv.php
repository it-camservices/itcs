...
// Create page object
$tbl_experiences_photos_blobview = new ctbl_experiences_photos_blobview();
$Page =& $tbl_experiences_photos_blobview;

// Page init
$tbl_experiences_photos_blobview->Page_Init();

// Page main
$tbl_experiences_photos_blobview->Page_Main();
?>
<?php
$tbl_experiences_photos_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class ctbl_experiences_photos_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 'tbl_experiences_photos_blobview';
	...
	//
	function ctbl_experiences_photos_blobview() {
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('tbl_experiences', 'x_photos');

		// Show thumbnail
		$bShowThumbnail = (@$_GET["showthumbnail"] == "1");
		if (@$_GET["thumbnailwidth"] == "" && @$_GET["thumbnailheight"] == "") {
			$iThumbnailWidth = 220; // Set default width
			$iThumbnailHeight = 160; // Set default height
		} else {
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('photos'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('photos'));
				}
				$objBinary->Value = $rs->fields('photos');
